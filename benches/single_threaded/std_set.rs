...
fn build<T: BenchValue>(values: &[T]) -> BTreeSet<T> {
...
pub fn bench_insert_batch<T: BenchValue>(
    ...
    group.bench_function(BenchmarkId::new(format!("std_batch_{insert_count}"), set_size), |b| {
        b.iter_batched_ref(
            || (build(base_values), insertions.to_vec()),
            |(set, insertion_batch)| {
...
pub fn bench_traversal<T: BenchValue>(group: &mut BenchmarkGroup<'_, WallTime>, set_size: usize, values: &[T]) {
    let mut set = None;
    group.bench_function(BenchmarkId::new("std", set_size), |b| {
        let set = set.get_or_insert_with(|| build(values));
        b.iter(|| {
...
pub fn bench_range<T: BenchValue>(group: &mut BenchmarkGroup<'_, WallTime>, set_size: usize, values: &[T]) {
