...
fn build<V: BenchMapValue>(entries: &[(u64, V)], node_capacity: usize) -> BTreeMap<u64, V> {
    let mut map = BTreeMap::with_maximum_node_size(node_capacity);
...
pub fn bench_insert_batch<V: BenchMapValue>(
    ...
    group.bench_function(BenchmarkId::new(id(node_capacity), map_size), |b| {
        ...
        b.iter_batched_ref(
            || (build(&base_entries, node_capacity), insertions.clone()),
            |(map, insertion_batch)| {
        ...
        b.iter_custom(|iterations| {
            ...
            for _ in 0..iterations {
                let insertion_batch = (0..insertions.len())
                    ...
                    .collect::<Vec<_>>();
                let mut map = build(&base_entries, node_capacity);
...
pub fn bench_get<V: BenchMapValue>(
    group: &mut BenchmarkGroup<'_, WallTime>,
    map_size: usize,
    node_capacity: usize,
    hit: bool,
    ...
    group.bench_function(BenchmarkId::new(id(node_capacity), map_size), |b| {
        let (map, queries) = fixture.get_or_insert_with(|| {
            let generator = ValueGenerator::new(map_size);
            let map = build(&generator.map_base_entries::<V>(), node_capacity);
            let queries = if hit {
                generator.hit_keys()
            } else {
                generator.miss_keys()
...
pub fn bench_remove<V: BenchMapValue>(group: &mut BenchmarkGroup<'_, WallTime>, map_size: usize, node_capacity: usize) {
    group.bench_function(BenchmarkId::new(id(node_capacity), map_size), |b| {
        let generator = ValueGenerator::new(map_size);
        let base_entries = generator.map_base_entries::<V>();
...
pub fn bench_traversal<V: BenchMapValue>(
    group: &mut BenchmarkGroup<'_, WallTime>,
    map_size: usize,
    node_capacity: usize,
) {
    ...
    group.bench_function(BenchmarkId::new(id(node_capacity), map_size), |b| {
        let map = map.get_or_insert_with(|| {
            let generator = ValueGenerator::new(map_size);
            build(&generator.map_base_entries::<V>(), node_capacity)
        });
...
pub fn bench_range<V: BenchMapValue>(group: &mut BenchmarkGroup<'_, WallTime>, map_size: usize, node_capacity: usize) {
